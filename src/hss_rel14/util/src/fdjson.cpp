...
class AvpDictionaryEntry {
 public:
  ...
  void init(const char* avp_name) {
    ...
    ret = fd_dict_search(
        fd_g_config->cnf_dict, DICT_AVP, AVP_BY_NAME_ALL_VENDORS, avp_name,
        &m_baseentry, ENOENT);
    if (ret != 0)
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Unable to find AVP dictionary entry  for [%s]",
    ...
    if (ret != 0)
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Unable to retrieve the dictionary data for the [%s] "
          "dictionary entry",
          __FILE__, __LINE__, avp_name));
    ...
    if (ret != 0)
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Unable to retrieve the dictionary for the [%s] "
          "dictionary entry",
          __FILE__, __LINE__, avp_name));

    /* get the dictionary entry associated with the derived type */
    ret = fd_dict_search(
        dict, DICT_TYPE, TYPE_OF_AVP, m_baseentry, &derivedtype, EINVAL);
    if (ret == 0) /* if found, then derived */
    {
      ...
      if (ret == 0) {
        if (!strcmp(derived_type_data.type_name, "Enumerated"))
          m_type = ADTEnumerated;
        else if (!strcmp(derived_type_data.type_name, "Time"))
          m_type = ADTTime;
        else if (!strcmp(derived_type_data.type_name, "Address"))
          m_type = ADTAddress;
        else if (!strcmp(derived_type_data.type_name, "UTF8String"))
          m_type = ADTUTF8String;
        else if (!strcmp(derived_type_data.type_name, "DiameterIdentity"))
          m_type = ADTDiameterIdentity;
        else if (!strcmp(derived_type_data.type_name, "DiameterURI"))
          m_type = ADTDiameterURI;
        else if (!strcmp(derived_type_data.type_name, "IPFilterRule"))
          m_type = ADTIPFilterRule;
        else {
 ...
 private:
  void _init(const char* avp_name) {
    ...
    if (it == dictEntries.end()) {
      ...
      if (it == dictEntries.end()) {
        ...
        ade->init(avp_name);
        std::pair<std::string, AvpDictionaryEntry*> data(
            ade->getAvpName(), ade);
        auto result = dictEntries.insert(data);
        if (result.second == false) {
          delete ade;
          throw runtimeInfo(string_format(
              "%s:%d - WARN - Unable to insert AvpDictionaryEntry for [%s]",
              __FILE__, __LINE__, avp_name));
        }
  ...
  void _addTo(msg_or_avp* reference) {
    ...
    if ((ret = fd_msg_avp_new(mBaseEntry, 0, &mAvp)) != 0)
      throw runtimeError(string_format(
          "%s:%d - ERROR - Error [%d] creating [%s] AVP", __FILE__, __LINE__,
          ret, mName));
    ...
    try {
      ...
      if ((ret = fd_msg_avp_add(reference, MSG_BRW_LAST_CHILD, mAvp)) != 0)
        throw runtimeError(string_format(
            "%s:%d - ERROR - Error [%d] adding [%s] AVP", __FILE__, __LINE__,
            ret, mName));
    } catch (...) {
      fd_msg_free(mAvp);
      mAvp = NULL;
...
#define THROW_DATATYPE_MISMATCH()                                              \
  {                                                                            \
    throw runtimeInfo(string_format(                                           \
        "%s:%d - INFO - Datatype mismatch for [%s] - expected datatype "       \
        "compatible with %s, JSON data type was %s",                           \
        __FILE__, __LINE__, name,                                              \
        avp.getType() == ADTUnknown ?                                          \
            "ADTUnknown" :                                                     \
            avp.getType() == ADTOctetString ?                                  \
            "ADTOctetString" :                                                 \
            avp.getType() == ADTI32 ?                                          \
            "ADTI32" :                                                         \
            avp.getType() == ADTI64 ?                                          \
            ...
            avp.getType() == ADTF64 ?                                          \
            "ADTF64" :                                                         \
            avp.getType() == ADTGrouped ?                                      \
            "ADTGrouped" :                                                     \
            avp.getType() == ADTAddress ?                                      \
            "ADTAddress" :                                                     \
            avp.getType() == ADTTime ?                                         \
            "ADTTime" :                                                        \
            avp.getType() == ADTUTF8String ?                                   \
            "ADTUTF8String" :                                                  \
            avp.getType() == ADTDiameterIdentity ?                             \
            "ADTDiameterIdentity" :                                            \
            avp.getType() == ADTDiameterURI ?                                  \
            "ADTDiameterURI" :                                                 \
            avp.getType() == ADTEnumerated ?                                   \
            "ADTEnumerated" :                                                  \
            avp.getType() == ADTIPFilterRule ? "ADTIPFilterRule" : "UNKNOWN",  \
        value.GetType() == RAPIDJSON_NAMESPACE::kNullType ?                    \
            "kNullType" :                                                      \
            value.GetType() == RAPIDJSON_NAMESPACE::kFalseType ?               \
            "kFalseType" :                                                     \
            value.GetType() == RAPIDJSON_NAMESPACE::kTrueType ?                \
            "kTrueType" :                                                      \
            value.GetType() == RAPIDJSON_NAMESPACE::kObjectType ?              \
            "kObjectType" :                                                    \
            value.GetType() == RAPIDJSON_NAMESPACE::kArrayType ?               \
            "kArrayType" :                                                     \
            value.GetType() == RAPIDJSON_NAMESPACE::kStringType ?              \
            "kStringType" :                                                    \
            value.GetType() == RAPIDJSON_NAMESPACE::kNumberType ? "kNumber" :  \
                                                                  "Unknown")); \
  }
...
static bool isHexString(const char* s, int len) {
  if (len % 2 != 0) {
    /*
    throw runtimeInfo(
       string_format("%s:%d - INFO - Length of HEX string (%d) not divisible by
    2",
  ...
  if (s[0] != '0' || !(s[1] == 'x' || s[1] == 'X')) {
    /*
    throw runtimeInfo(
       string_format("%s:%d - INFO - First 2 characters of hex string are not
    \"0x\" or \"0X\"",
       __FILE__, __LINE__)
    );
    */
  ...
  for (int i = 2; i < len; i++)
    if (!ISHEXDIGIT(s[i])) {
      /*
      throw runtimeInfo(
         string_format("%s:%d - INFO - invalid character [%c] found in string of
      hex digits",
...
static void fdJsonAddAvps(
    AVP& avp, const RAPIDJSON_NAMESPACE::Value& element,
    void (*errfunc)(const char*));
static void fdJsonAddAvps(
    msg_or_avp* reference, const RAPIDJSON_NAMESPACE::Value& element,
    void (*errfunc)(const char*));

static void fdJsonAddAvp(
    msg_or_avp* reference, const char* name,
    const RAPIDJSON_NAMESPACE::Value& value, void (*errfunc)(const char*)) {
  AVP avp(name);
  ...
  switch (value.GetType()) {
    ...
    case RAPIDJSON_NAMESPACE::kTrueType: {
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Invalid format (true/false) for [%s] in JSON block, "
          "ignoring",
          __FILE__, __LINE__, name));
      break;
    }
    case RAPIDJSON_NAMESPACE::kNullType: {
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Invalid NULL for [%s] in JSON block, ignoring",
          __FILE__, __LINE__, name));
      break;
    }
    ...
    case RAPIDJSON_NAMESPACE::kStringType: {
      if (avp.getBaseType() != AVP_TYPE_OCTETSTRING) THROW_DATATYPE_MISMATCH();
      ...
      if (avp.getType() == ADTAddress) {
        ...
        if (inet_pton(AF_INET, value.GetString(), &((sSA4*) &ss)->sin_addr) ==
            1) {
          *(uint16_t*) abuf = htons(1);
          memcpy(abuf + 2, &((sSA4*) &ss)->sin_addr.s_addr, 4);
          avp.set((uint8_t*) abuf, 6);
        } else if (
            inet_pton(AF_INET6, value.GetString(), &((sSA6*) &ss)->sin6_addr) ==
            1) {
        ...
        if (isHexString((const char*) value.GetString(), rawlen)) {
          /*
           * hex string format is "0x" or "0X" followed by an even number of hex
           * characters binlen is equal to the final length + 1
           */
           ...
           * start index at 1 (first hex digit divided by number of digits per
           * byte, 2 / 2 = 1) to start at the first hex digit
           */
          for (uint32_t i = 1; i < binlen; i++)
            avp.getBuffer().get()[i - 1] =
                (HEX2BIN(p[i * 2]) << 4) + HEX2BIN(p[i * 2 + 1]);
    ...
    default: {
      throw runtimeInfo(string_format(
          "%s:%d - INFO - invalid JSON data type encountered in element [%s]",
          __FILE__, __LINE__, name));
      break;
    }
  }
...
std::string fdJsonBinaryToHex(const unsigned char* buffer, size_t len) {
  ...
  for (size_t i = 0; i < len; i++)
    ss << hexDigits[UPPERNIBBLE(buffer[i])]
       << hexDigits[LOWERNIBBLE(buffer[i])];

  // return the hex string
  return ss.str();
}

static std::string fdJsonTimeToStr(
    struct dict_avp_data* dictData, const unsigned char* buffer, size_t len) {
  STime t;
  ntp_time_t ntp;
  std::string ts;
  ...
  if (len != 4)
    throw runtimeInfo(string_format(
        "%s:%d - INFO - The length for Time is not 4 in AVP [%s]", __FILE__,
        __LINE__, dictData->avp_name));

  ntp.fraction = 0;
  ntp.second   = (((unsigned long) (buffer[0] & 0xff)) << 24) +
...
static std::string fdJsonAddressToStr(
    struct dict_avp_data* dictData, const unsigned char* buffer, size_t len) {
  sSS ss;
  ...
  switch (addressType) {
    case 1:  // IPV4
    {
      if (len != 6)
        throw runtimeInfo(string_format(
            "%s:%d - INFO - The length for an IPV4 address is not 6 in AVP "
            "[%s]",
    ...
    case 2:  // IPV6
    {
      if (len != 18)
        throw runtimeInfo(string_format(
            "%s:%d - INFO - The length for an IPV4 address is not 18 in AVP "
            "[%s]",
            __FILE__, __LINE__, dictData->avp_name));
    ...
    default: {
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Unrecognized address type [%d] in AVP [%s]", __FILE__,
          __LINE__, addressType, dictData->avp_name));
    }
  }
...
static void fdJsonAddMembers(
    msg_or_avp* ref, RAPIDJSON_NAMESPACE::Value& object,
  ...
  if (fd_msg_browse_internal(
          ref, MSG_BRW_FIRST_CHILD, (msg_or_avp**) &a, NULL) != 0)
    return;
  ...
  do {
    ...
    if (ret != 0)
      throw runtimeError(string_format(
          "%s:%d - INFO - error fd_msg_model() returned %d", __FILE__, __LINE__,
          ret));
    ...
    if (ret != 0)
      throw runtimeError(string_format(
          "%s:%d - INFO - error fd_dict_getval() returned %d", __FILE__,
          __LINE__, ret));
    ...
    if (dictData.avp_basetype == AVP_TYPE_OCTETSTRING) {
      ...
      /* get the dictionary entry associated with the derived type */
      ret = fd_dict_search(
          dict, DICT_TYPE, TYPE_OF_AVP, dictEntry, &derivedtype, EINVAL);
      if (ret == 0) /* if found, then derived */
    ...
    if (fd_msg_avp_hdr(a, &hdr) == 0) {
        ...
        case AVP_TYPE_OCTETSTRING: {
          if (isDerived) {
            if (!strcmp(derivedData.avp_name, "Address")) {
              std::string address = fdJsonAddressToStr(
                  &dictData, hdr->avp_value->os.data, hdr->avp_value->os.len);
            ...
            std::string hex = fdJsonBinaryToHex(
      ...
      throw runtimeInfo(string_format(
          "%s:%d - INFO - Unable to retrieve the AVP header for [%s]", __FILE__,
          __LINE__, dictData.avp_name));
    }
  } while (fd_msg_browse_internal(a, MSG_BRW_NEXT, (msg_or_avp**) &a, NULL) ==
           0);
}
...
static std::string fdJsonGetName(msg_or_avp* ref) {
  ...
  bool isAvp = fd_msg_browse_internal(
                   ref, MSG_BRW_PARENT, (msg_or_avp**) &parent, NULL) == 0;

  ret = fd_msg_model(ref, &model);
  if (ret != 0)
    throw runtimeError(string_format(
        "%s:%d - ERROR - error fd_msg_model() returned %d", __FILE__, __LINE__,
        ret));
  ...
  if (isAvp) {
    ...
    if (ret != 0)
      throw runtimeError(string_format(
          "%s:%d - ERROR - error fd_dict_getval() returned %d", __FILE__,
          __LINE__, ret));
...
void parseRecursive(
    std::string scope, RAPIDJSON_NAMESPACE::Value::ConstMemberIterator object,
  ...
  if (object->value.IsObject()) {
    for (auto it = object->value.MemberBegin(); it != object->value.MemberEnd();
         ++it) {
    ...
    // std::cout << "Unsupported: " << inElement << object->name.GetString() <<
    // std::endl ;
  }
}
...
bool fdJsonGetApnValueFromSubData(std::string json, std::string& apn) {
  ...
  if (!json.c_str()) {
    std::cout << __FILE__ ":" << __LINE__
              << " - ERROR - Error parsing JSON string" << std::endl;
    return false;
  }

  if (doc.Parse<RAPIDJSON_NAMESPACE::kParseNoFlags>(json.c_str())
          .HasParseError()) {
    std::cout << __FILE__ ":" << __LINE__
              << " - ERROR - Error parsing JSON string" << std::endl;
    return false;
