...
void dumpUlrTimers() {
  ...
  for (long idx = 0; ulrTimers[idx].ulr1 > 0; idx++) {
    fprintf(
        fp, "%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
...
IDRRreq::IDRRreq(
    Application& app, FDMessageRequest* cir_req, EvenStatusMap* evt_map,
    RIRBuilder* rirbuilder, std::string& imsi, std::string& msisdn)
    : INSDRreq(app),
      cir_req(cir_req),
      evt_map(evt_map),
...
void IDRRreq::processAnswer(FDMessageAnswer& ans) {
  ...
  if (ida.result_code.get(ida_result_code)) {
    ...
    StatsHss::singleton().registerStatResult(
        stat_hss_idr, vendor_code, ida_result_code);
  }
...
int Application::sendINSDRreq(
  ...
  if (!imsi_attached || !mme_reachable) {
    if (rir_builder == NULL) {
      ...
      printf(
          "****************Application::sendINSDRreq::MME_DOWN: %s\n\n",
  ...
  INSDRreq* s = createINSDRreq(
      cir_monevtcfg, imsi, cir_req, evt_map, imsi_info, rir_builder);
...
INSDRreq* Application::createINSDRreq(
  ...
  INSDRreq* s = new IDRRreq(
      *this, cir_req, evt_map, rir_builder, imsi, imsi_info.str_msisdn);
...

  {
    ...
    FDAvp subscription_data(
        getDict().avpSubscriptionData(),
...
int PUURcmd::process(FDMessageRequest* req) {
  ...
  if (DIAMETER_ERROR_IS_VENDOR(result_code) && experimental) {
    ...
    StatsHss::singleton().registerStatResult(
        stat_hss_pur, VENDOR_3GPP, result_code);
  } else {
...
ULRProcessor::ULRProcessor(
    FDMessageRequest& req, s6as6d::Application& app, s6as6d::Dictionary& dict)
...
void ULRProcessor::on_ulr_callback(CassFuture* future, void* data) {
  ...
  int actionofs = action->getAction() == ULRDB_GET_IMSI_INFO ?
                      0 :
...
#ifdef TRACK_EXECUTION
      printf(
          "%lld,%s,%p,%s,m_dbissued=%d,m_msgissued=%d\n",
          STIMER_GET_CURRENT_TIME, __PRETTY_FUNCTION__, this,
          phases[phase - ULRSTATE_BASE], m_dbissued - adjustment, m_msgissued);
#endif
...
std::cout << pthis->m_nextphase << std::endl;
      switch (pthis->m_nextphase) {
        ...
        default: {
          Logger::s6as6d().warn(
              "Unrecognized ULRSTATE (%u)", pthis->m_nextphase);
          pthis = NULL;
          break;
        }
      }
...
void ULRProcessor::getEvents() {
  ...
  for (auto it = m_evtIdLst.begin(); it != m_evtIdLst.end(); ++it) {
    if (scef_id != (*it)->scef_id && scef_ref_ids.size() > 0) {
      ...
      if (!success) {
        DB_OP_COMPLETE(
            ULRDB_GET_EVNTS_EVNTIDS, m_dbexecuted, m_dbresult, false);
        atomic_dec_fetch(m_dbissued);
        return;
      }
...
void ULRProcessor::phase1() {
  ...
  if (m_new_info.imsi.length() > IMSI_LENGTH) {
    ...
    m_ans.send();
    StatsHss::singleton().registerStatResult(
        stat_hss_ulr, 0, ER_DIAMETER_INVALID_AVP_VALUE);
  ...
  result = m_app.dynamodb().getImsiInfo(
   ...
   if (result) {
    ...
    if (!result) {
      ...
      DB_OP_COMPLETE(
          ULRDB_GET_EVNTIDS_EXTIDS, m_dbexecuted, m_dbresult, result);
  ...
  if (result) {
    ...
    m_ans.send();
    StatsHss::singleton().registerStatResult(
        stat_hss_ulr, VENDOR_3GPP, DIAMETER_ERROR_USER_UNKNOWN);
    m_nextphase = ULRSTATE_PHASEFINAL;
  }
}
...
void ULRProcessor::phase2() {
  ...
  if (u32 != 1004 ||
    ...
    m_ans.send();
    StatsHss::singleton().registerStatResult(
        stat_hss_ulr, VENDOR_3GPP, DIAMETER_ERROR_RAT_NOT_ALLOWED);
    m_nextphase = ULRSTATE_PHASEFINAL;
    return;
  }
  ...
  if (FLAG_IS_SET(u32, ULR_INITIAL_ATTACH_IND)) {
    ...
    if (!m_ddborig_info.mmehost.empty() && !m_ddborig_info.mmerealm.empty()) {
      if (m_ddborig_info.mmehost != m_new_info.mmehost) {
        ...
        er.add(m_dict.avpVendorId(), VENDOR_3GPP);
        er.add(
            m_dict.avpExperimentalResultCode(),
...
void ULRProcessor::phase3() {
  ...
  StatsHss::singleton().registerStatResult(
      stat_hss_ulr, 0, ER_DIAMETER_SUCCESS);
      ...
      if (m_ddborig_info.visited_plmnid != m_new_info.visited_plmnid) {
        for (DAEventList::iterator it_evt = m_evtLst.begin();
          ...
          if ((*it_evt)->monitoring_type == ROAMING_STATUS_EVT) {
            ...
            s->add(
                fdHss.gets6tApp()->getDict().avpSessionId(), s->getSessionId());
  ...
  switch (phase) {
    ...
    case AIRSTATE_PHASEFINAL: {
      ...
      printf(
          "%lld,%s,%p,%s,m_dbissued=%d,m_msgissued=%d\n",
          STIMER_GET_CURRENT_TIME, __PRETTY_FUNCTION__, this,
          phases[phase - AIRSTATE_BASE], m_dbissued - adjustment, m_msgissued);
#endif
      ready = ((m_dbissued - adjustment) <= 0 && m_msgissued == 0);
      break;
    }
...

  {
    ...
    while (pthis && pthis->phaseReady(pthis->m_nextphase)) {
      ...
      switch (pthis->m_nextphase) {
        ...
        default: {
          Logger::s6as6d().warn(
              "Unrecognized AIRSTATE (%u)", pthis->m_nextphase);
          pthis = NULL;
          break;
        }
      }
...
void AIRProcessor::phase1() {
  ...
  if (m_imsi.length() > IMSI_LENGTH) {
    ...
    m_ans.send();
    StatsHss::singleton().registerStatResult(
        stat_hss_air, 0, ER_DIAMETER_INVALID_AVP_VALUE);
    m_nextphase = ULRSTATE_PHASEFINAL;
    return;
  }
  ...
  if (!eutran_avp_found) {
    if (m_air.requested_utran_geran_authentication_info
      ...
      er.add(m_dict.avpVendorId(), VENDOR_3GPP);
      er.add(
          m_dict.avpExperimentalResultCode(), DIAMETER_ERROR_RAT_NOT_ALLOWED);
      m_ans.add(er);
      m_ans.send();
      StatsHss::singleton().registerStatResult(
...
void AIRProcessor::phase2() {
  ...
  for (uint32_t i = 0; i < m_num_vectors; i++) {
    ...
    eurtran_vector.add(
        m_dict.avpRand(), m_vector[i].rand, sizeof(m_vector[i].rand));
    eurtran_vector.add(
        m_dict.avpXres(), m_vector[i].xres, sizeof(m_vector[i].xres));
    eurtran_vector.add(
        m_dict.avpAutn(), m_vector[i].autn, sizeof(m_vector[i].autn));
    eurtran_vector.add(
        m_dict.avpKasme(), m_vector[i].kasme, sizeof(m_vector[i].kasme));
  ...
  m_ans.send();
  StatsHss::singleton().registerStatResult(
      stat_hss_air, 0, ER_DIAMETER_SUCCESS);
