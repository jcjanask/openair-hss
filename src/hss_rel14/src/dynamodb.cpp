...
bool DynamoDb::getImsiSecData(std::string imsi, DDBImsiSec& sec, void* data) {
  ...
  std::cout << "Get Imsi Sec Data successful.";
   return true;
}
...
bool DynamoDb::updateRandSqn(
  ...
  SqnU64Union eu;
  SQN_TO_U64(sqn, eu);  // Convert SQN to unsigned int 64 bytes
  ...
  std::cout << "Rand and Sqn update successful" << std::endl;
  return true;
}
...
bool DynamoDb::getExtIdsFromImsi(
    std::string imsi, DDBExtIdList& extids, void* data) {
  Aws::Vector<Aws::String> attributesToGet;
  ...
  std::cout << "Get ExtIDs successful" << std::endl;
  return true;
}

bool DynamoDb::getEventIdsFromMsisdn(
    int64_t& msisdn, DDBEventIdList& ddbel, void* data) {
  Aws::Client::ClientConfiguration clientConfig;
  clientConfig.region = "us-west-2";
  Aws::DynamoDB::DynamoDBClient dynamoClient(clientConfig);
  ...
  req.WithProjectionExpression("scef_id, scef_ref_id");  // get event ids

  Aws::DynamoDB::Model::QueryOutcome result = dynamoClient.Query(req);
  return result.IsSuccess();
}

bool DynamoDb::getMmeIdFromHost(
    std::string mmehost, int32_t& m_mmeidentity, void* data) {
  Aws::Client::ClientConfiguration clientConfig;
  Aws::DynamoDB::DynamoDBClient dynamoClient(clientConfig);
  Aws::DynamoDB::Model::QueryRequest req;
  ...
  req.SetIndexName("mmehost");            // search based on gsi mmehost
  req.WithProjectionExpression("mmeid");  // search for mmeid
  ...
  std::cout << "Get MME ID from Host successful."; 
  return true;
}

bool DynamoDb::updateLocation(
    DDBImsiInfo& location, uint32_t present_flags, int32_t idmmeidentity,
    void* data) {
  // Client configs
  Aws::Client::ClientConfiguration clientConfig;
  clientConfig.region = "us-west-2";
  ...
  std::cout << "Location was updated" << std::endl;
  return true;
}

//============================================= NEED WORK, NON ESSENTIAL FOR INITIAL UE ATTACH
...
bool DynamoDb::purgeUe(std::string imsi) {
  ...
  std::cout << "Purge successful." << std::endl;
  return true;
}
...
//============================================= OPC keys need to be checked for all users ===========
...
bool DynamoDb::updateOpc(std::string& imsi, std::string& opc) {
  ...
  std::cout << "Update of opc successful";
  return true;
}
...
//Due to many calls to get item from imsi
